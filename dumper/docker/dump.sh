...
for layer in $(pwd)/data/image/*/layer.tar; do
    ...
    if [ -n "$(tar -tf ${layer} | grep 'java-10-oracle')" ]; then echo found java-10-oracle in ${layer}; LAYERS+=(${layer}); fi
done
...
tar_directories=($(tar tf data/layer.tar | grep -E '.*/$' | grep -v 'java' | sort -r -n))
